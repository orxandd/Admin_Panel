<?php
 class Portfolio_model_woc_ml extends CI_Model {
...
//     portfolio listinin hissesi
     public function get_portfolio_list(){
         return $this->db->order_by("id", "DESC")->get("portfolio_woc_ml")->result_array();
     }

     public function get_portfolio_list_e($name){
         return $this->db->where('category_name',$name)->order_by("id", "DESC")->get("portfolio_woc_ml")->result_array();
     }

     public function get_portfolio_list_single($where){
         return $this->db->where($where)->get("portfolio_woc_ml")->row_array();
     }

     public function portfolio_list_add($data){
         $this->db->insert("portfolio_woc_ml", $data);
     }

     public function portfolio_list_update($where, $data){
         $this->db->where($where)->update("portfolio_woc_ml", $data);
     }

     public function portfolio_list_delete($where){
         $this->db->where($where)->delete("portfolio_woc_ml");
